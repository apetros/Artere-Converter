...
def from_artere(artereFile):
    ...
    with open(artereFile) as Artere_file:
        ...
        for Line in Lines:
            ...
            if len(line) > 0:
                if line[0] == 'SLACK':
                    slack_bus_name = line[1]
                    slack_bus_list.append(slack_bus_name)
                global frequency
                if line[0] == 'FNOM':
            ...
            if len(line) > 0:
                ...
                if line[0] == 'BUS':
                    ...
                    try:
                        ...
                        pass
                    bus_names_list.append(bus_name)

                    # Create a bus
                    Bus_dict[bus_name] = {}
                    Bus_dict[bus_name]["name"] = bus_name
                    Bus_dict[bus_name]["voltage"] = vn_kv
                    ...
                    if Bshunt != 0:
                        ...
                        shunt_list.append(created_shunt)
                ...
                if line[0] == 'LINE':
                    ...
                    line_voltage = Bus_dict[from_bus]["voltage"]
                    ...
                    created_line = f'pp.create_line_from_parameters(net, from_bus={Bus_dict[from_bus]["name"]}, to_bus={Bus_dict[to_bus]["name"]}, name="{line_name}"' \
                                   f', length_km=1, r_ohm_per_km={R_Ohm}, x_ohm_per_km={X_Ohm}, max_i_ka={Imax}, c_nf_per_km={Capacitance}, in_service={in_service})\n'
                    line_list.append(created_line)
                ...
                if line[0] == 'TRANSFO':
                    ...
                    if Bus_dict[from_bus]['voltage'] > Bus_dict[to_bus]['voltage']:
                    ...
                    if N_ratio == 100:
                        created_transformer = f'pp.create_transformer_from_parameters(net, hv_bus={Bus_dict[high_lvl_bus]["name"]}, lv_bus={Bus_dict[low_lvl_bus]["name"]}, sn_mva={Snom}, ' \
                                              f'name={transfo_name}, shift_degree={PHI}, vn_hv_kv={Bus_dict[high_lvl_bus]["voltage"]}, vn_lv_kv={Bus_dict[low_lvl_bus]["voltage"]}, vkr_percent={vkr_percent}, vk_percent={vk_percent}, pfe_kw=0, i0_percent={i0_percent}, in_service={in_service})\n'
                    else:
                        created_transformer = f'pp.create_transformer_from_parameters(net, hv_bus={Bus_dict[high_lvl_bus]["name"]}, lv_bus={Bus_dict[low_lvl_bus]["name"]}, sn_mva={Snom}, name={transfo_name}, ' \
                                              f'shift_degree={PHI}, vn_hv_kv={Bus_dict[high_lvl_bus]["voltage"]}, vn_lv_kv={Bus_dict[low_lvl_bus]["voltage"]}, vkr_percent={vkr_percent}, vk_percent={vk_percent},' \
                    ...
                    transformer_list.append(created_transformer)
                ...
                if line[0] == 'TRFO':
                    ...
                    try:
                        if Bus_dict[CON_BUS]['voltage'] == Bus_dict[high_lvl_bus]['voltage']:
                            tap_side = "\"hv\""
                        elif Bus_dict[CON_BUS]['voltage'] == Bus_dict[low_lvl_bus]['voltage']:
                            tap_side = "\"lv\""
                        else:
                            tap_side = None
                    except:
                    ...
                    if N_ratio != 100 and NBPOS == 0 and N_First == 0 and N_Last == 0:
                        ...
                        # Create a string
                        created_transformer = f'pp.create_transformer_from_parameters(net, hv_bus={Bus_dict[high_lvl_bus]["name"]}, lv_bus={Bus_dict[low_lvl_bus]["name"]}, sn_mva={Snom}, name="{transfo_name}", ' \
                                              f'vn_hv_kv={Bus_dict[high_lvl_bus]["voltage"]}, vn_lv_kv={Bus_dict[low_lvl_bus]["voltage"]}, vkr_percent={vkr_percent}, vk_percent={vk_percent}, pfe_kw=0, i0_percent={i0_percent}, ' \
                    ...
                    # if N_ratio , N_First, N_last are equal:
                        ...
                        # Create a string
                        created_transformer = f'pp.create_transformer_from_parameters(net, hv_bus={Bus_dict[high_lvl_bus]["name"]}, lv_bus={Bus_dict[low_lvl_bus]["name"]}, sn_mva={Snom}, name="{transfo_name}", ' \
                                              f'vn_hv_kv={Bus_dict[high_lvl_bus]["voltage"]}, vn_lv_kv={Bus_dict[low_lvl_bus]["voltage"]}, vkr_percent={vkr_percent}, vk_percent={vk_percent}, pfe_kw=0, ' \
                ...
                if line[0] == 'GENER':
                    ...
                    if VIMP == "0":
                        created_sgen = f'pp.create_sgen(net, bus={Bus_dict[bus_connected]["name"]}, p_mw={Pproduction}, q_mvar={Qproduction}, sn_mva={Snom}, name="{gener_name}", max_q_mvar={Qmax}, min_q_mvar={Qmin}, in_service={in_service})\n'
                        gen_list.append(created_sgen)

                    else:
                        if gener_name in slack_bus_list:
                            created_gen = f'pp.create_gen(net, p_mw={Pproduction}, max_q_mvar={Qmax}, min_q_mvar={Qmin}, sn_mva={Snom}, bus={Bus_dict[bus_connected]["name"]}, vm_pu={VIMP}, name="{gener_name}", slack={True}, in_service={in_service})\n'
                            gen_list.append(created_gen)
                            ...
                            created_gen = f'pp.create_gen(net, p_mw={Pproduction}, max_q_mvar={Qmax}, min_q_mvar={Qmin}, sn_mva={Snom}, bus={Bus_dict[bus_connected]["name"]}, vm_pu={VIMP}, name="{gener_name}", slack={False}, in_service={in_service})\n'
                            gen_list.append(created_gen)
                ...
                if line[0] == 'SWITCH':
                    ...
                    created_switch = f'pp.create_switch(net, bus={Bus_dict[first_bus]["name"]}, name="{switch_name}" ,element={second_bus}, et="b" ,closed={in_service})\n'
                    switch_list.append(created_switch)
